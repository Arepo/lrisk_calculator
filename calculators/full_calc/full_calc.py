...
def full_markov_chain():
    ...
    # (pre_equilibrium, preindustrial, industrial, perils, multiplanetary) * max civilisations,
    # minus the three preperils states in our current civilisation)
    ...
    pre_equilibrium_rows = [
        _zero_probabilities()
        # ^Other pre_equilibrium states
        + [preperils.preindustrial_given_pre_equilibrium(k,k1)
           for k1 in preperils_civilisation_range]
    ...
    perils_rows = [[perils_chain(k).pre_equilibrium_given_perils(k1)
                    for k1 in range(1, constant.MAX_CIVILISATIONS)]
                # ^Transition probabilities to future pre_equilibrium states
                + [perils_chain(k).preindustrial_given_perils(k1)
                   for k1 in range(1, constant.MAX_CIVILISATIONS)]
    ...
    multiplanetary_rows = [[multiplanetary_chain(k).pre_equilibrium_given_multiplanetary(k1)
                            for k1 in range(1, constant.MAX_CIVILISATIONS)]
                            # ^pre_equilibrium states
                            + [multiplanetary_chain(k).preindustrial_given_multiplanetary(k1)
                               for k1 in range(1, constant.MAX_CIVILISATIONS)]
                            # ^Preindustrial states
                            + [multiplanetary_chain(k).industrial_given_multiplanetary(k1)
                               for k1 in range(1, constant.MAX_CIVILISATIONS)]
                            # ^Industrial states
                            + [multiplanetary_chain(k).perils_given_multiplanetary(k1)
                               for k1 in range(0, constant.MAX_CIVILISATIONS)]
                            # ^Perils states (which include the current civilisation)
                            + _zero_probabilities() + [0]
                            # ^Multiplanetary states (which potentially include the current
                            # civilisation)
    ...
    pre_equilibrium_names = ['pre_equilibrium-' + str(index[0] + 1)
                             for index in enumerate(pre_equilibrium_rows)]
...
print(f"""  From this breakpoint, you can query the MarkovChain (variable name mc) object as
    ...
    interstellar is
    ~{round(mc.absorption_probabilities()[1][(constant.MAX_CIVILISATIONS - 1) * 3] * 100)}%.
