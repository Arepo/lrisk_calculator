...
def update_transitions(transition_name, transitions, remainder_function):
    other_transitions = [transition for transition in transitions if transition != transition_name]
...
specifiable_transitions_from_preindustrial = [
    'industrial_given_preindustrial'
]

specifiable_transitions_from_industrial = [
    'future_perils_given_industrial'
]
...
for session_value in (specifiable_transitions_from_preindustrial +
                      specifiable_transitions_from_industrial +
                      specifiable_transitions_from_present_perils +
                      specifiable_transitions_from_future_perils +
                      specifiable_transitions_from_multiplanetary +
    ...
    if session_value not in st.session_state:
        st.session_state[session_value] = 0.1
...
def make_on_change_present_perils_callback(transition_name):
    ...
    def callback():
        update_transitions(transition_name, specifiable_transitions_from_present_perils, present_perils_remainder)
    return callback
...
def make_on_change_future_perils_callback(transition_name):
    # Create a function to allow us to pass the transition name to the callback
    def callback():
        update_transitions(transition_name, specifiable_transitions_from_future_perils, future_perils_remainder)
    return callback
...
def make_on_change_multiplanetary_callback(transition_name):
    # Create a function to allow us to pass the transition name to the callback
    def callback():
        update_transitions(transition_name, specifiable_transitions_from_multiplanetary, multiplanetary_remainder)
    return callback
